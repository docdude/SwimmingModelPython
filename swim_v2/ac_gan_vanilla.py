...
def build_conditional_generator(latent_dim, num_styles,  output_shape, generator_parameters, use_seed=True, use_stroke=True,):
    ...
    # Embeddings
    swim_style_embedding = tf.keras.layers.Embedding(input_dim=num_styles, output_dim=10,name=f'gen_swim_style_embed')(swim_style_input)
    swim_style_embedding = tf.keras.layers.Flatten(name=f'gen_swim_style_flatten')(swim_style_embedding)  # shape (None, 10)
    
    if use_stroke:
        # Expand stroke label to match timesteps
        stroke_label_expanded = tf.keras.layers.RepeatVector(output_shape[0],name='gen_repeat_vector')(stroke_label_input)  # (B, 180, 1)
        stroke_label_embedding = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Embedding(input_dim=2, output_dim=5, name='gen_stroke_embed'),  # Embed each timestep's label
            name='gen_time_dist'
        )(stroke_label_expanded)  # (B, 180, 5)
        stroke_label_embedding = tf.keras.layers.Reshape((-1, 5),name='gen_time_dist_reshape')(stroke_label_embedding)  # (B, 180*5)
        
        merged = tf.keras.layers.Concatenate(name=f'gen_swim_stroke_concat')([noise_input, swim_style_embedding, stroke_label_embedding])
    else:
        merged = tf.keras.layers.Concatenate(name=f'gen_swim_concat')([noise_input, swim_style_embedding])
    ...
    x = tf.keras.layers.Dense(
        ...
        name=f'gen_initial_dense'
    )(merged)
    x = tf.keras.layers.Reshape((initial_dim, initial_filters), name=f'gen_initial_reshape')(x)
    x = tf.keras.layers.BatchNormalization(name=f'gen_initial_bn')(x)
    x = tf.keras.layers.LeakyReLU(0.2, name=f'gen_initial_activation')(x)

    # Upsampling layers
    current_dim = initial_dim
    for i in range(num_cl):
        if current_dim < output_shape[0]:
            x = tf.keras.layers.UpSampling1D(2, name=f'gen_upsampling_{i}')(x)
            current_dim *= 2
    ...
    if current_dim != output_shape[0]:
        if current_dim > output_shape[0]:
            ...
            x = tf.keras.layers.Cropping1D(cropping=(crop_size, crop_size), name=f'gen_cropping')(x)
        else:
            x = tf.keras.layers.ZeroPadding1D(padding=(0, output_shape[0] - current_dim), name=f'gen_zeropad')(x)
...
def main():
    ...
    test_noise = tf.random.normal([1, latent_dim])  # shape (1, 100)
    test_style = tf.constant([[1]], dtype=tf.int32)  # shape (1,1) just an example style = 1
    test_stroke = tf.random.uniform([1, 180, 1], minval=0, maxval=2, dtype=tf.int32)  # (1, 180, 1)

    test_output = generator([test_noise, test_style, test_stroke])
