...
save_path = '/Users/juanloya/Documents/SwimmingModelPython/swim_v2/tutorial_save_path_epochDense_BiLSTM1_weighted'
...
training_parameters = {'lr':              0.0005,
                       ...
                       'beta_2':          0.999,
                       'batch_size':      128,
                       'max_epochs':      50,      # Keeping small for quick testing
...
# Define weights dynamically (can be adjusted or passed as input)
def stroke_loss_with_weights(y_true, y_pred):
    # Define weights for stroke (1) and no-stroke (0)
    stroke_weight = 10.0  # Adjust as necessary
    no_stroke_weight = 1.0

    # Compute weights based on y_true
    weights = tf.where(tf.equal(y_true, 1), stroke_weight, no_stroke_weight)

    # Ensure weights and y_true are float32 and expand dimensions if necessary
    weights = tf.cast(weights, dtype=tf.float32)
    y_true = tf.cast(y_true, dtype=tf.float32)

    # Expand weights if needed to match y_pred
    weights = tf.broadcast_to(weights, tf.shape(y_pred))
...
for (i, user_test) in enumerate(users_test):
    ...
    model = test_lstm.lstm_model(input_shape)
    ...
    x_val_combined, y_val_sparse, y_val_cat, y_stroke_val, y_val_combined, val_sample_weights, val_stroke_mask = swimming_data.get_windows_dict(
