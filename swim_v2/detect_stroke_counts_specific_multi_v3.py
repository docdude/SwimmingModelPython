...
def preprocess_sensor_data(normalized_window, label, config=None):
    """
...
def detect_peaks(filtered_sensor_data_list, base_prominence=0.5, raw_sensor_data=None, distance=5, config=None):
    """
    ...
    for sensor_data in filtered_sensor_data_list:
        ...
        #style = sensor_data.get("style", None)  # Assuming style is stored in sensor_data
...
def process_recording(args):
    user, rec, swimming_data, data_parameters, total_windows, config, progress_queue, position, output_dir = args
    ...
    try:
        ...
        window_data = extract_window_data_and_predictions(
            swimming_data, data_parameters, user, rec
        )
...
def count_strokes_by_style(normalized_windows, raw_windows, labels, config=None, window_pbar=None):
    """
    Count strokes using style-specific peak detection logic.

    Parameters:
    -----------
    normalized_windows : list of ndarray
        The normalized windows of sensor data.
    raw_windows : list of ndarray
        The raw windows of sensor data.
    config : Config, optional
        Configuration object for debugging flags.
    window_pbar : callable, optional
        Progress bar function to call during processing.

    Returns:
    --------
    stroke_counts : dict
        Counts of strokes for each style.
    stroke_labels : ndarray
        Labels for strokes detected in the dataset.
    """
    ...
    for i, (normalized_window, raw_window, label) in enumerate(zip(normalized_windows, raw_windows, labels)):
...
def count_strokes_by_style(normalized_windows, raw_windows, config=None, window_pbar=None):
...
def main():
    ...
    config.DEBUG_COUNT_STROKES = True
    ...
    with tqdm(
        ...
        for user in users:
            ...
            mp_args = [
                (
                    user, rec, swimming_data, data_parameters, 
