...
swimming_data.sliding_window_locs(
    win_len=data_parameters['win_len'], slide_len=0#slide_len=data_parameters['slide_len']
)
      ...
      #  windows = np.zeros(
       ...
       # )
        ...
        for iii in range(len(win_starts)):
            ...
            # Check if there is any label 6(kick) or label -1(unknown) and mask the window (optional)
            #if np.any(win_labels == 6) or np.any(win_labels == -1):
        ...
        y_pred_windows_strokes = y_pred_windows
        ...
        dynamic_prominence = utils.calculate_dynamic_prominence(y_pred_windows_strokes_flat)
        
        insights, _, _ = utils.analyze_peak_characteristics(y_pred_windows_strokes_flat)
        dynamic_prominence = insights['avg_peak_prominence']
        all_peaks = utils.detect_strokes_dynamic_prominence(y_pred_windows_strokes_flat, height=0.2, width=4, min_distance=None, dynamic_prominence=0.40)
        ...
        x = win_mids
        x_new = np.arange(0, len(y_true_raw_strokes))
   ...
   #     if data_parameters['label_type'] == 'sparse':
    ...
    #        y_pred_raw_swim_style = utils.resample(x, y_pred_windows_swim_style.T, x_new, kind='nearest').T
       # y_pred_raw_swim_style = utils.resample(x, y_pred_windows_swim_style.T, x_new, kind='nearest').T
        ...
        #raw_stroke_indices = utils.find_adaptive_stroke_indices(win_starts, win_stops, y_pred_windows_strokes, global_percentile=62, local_margin=0.001)
        insights, raw_stroke_results = utils.analyze_predictions(y_pred_windows_strokes, win_starts, win_stops)
        print("Peak Insights:", insights)
        # Output the detected raw stroke indices
        peak_indices_list = [window['peak_indices'] for window in raw_stroke_results]
        print("All Peak Indices:", peak_indices_list)
