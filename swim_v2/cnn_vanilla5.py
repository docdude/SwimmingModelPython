...
def cnn_model(input_shape, model_parameters, use_seed=True, output_bias=None):
    ...
    # Reshape shared features to match Conv1D input requirements
    stroke_label_branch = tf.keras.layers.Reshape((7, 64))(shared_features)  # (None, 7, 64)

    # Feature aggregation with Conv1D
    stroke_label_branch = tf.keras.layers.Conv1D(
        filters=64,
        kernel_size=3,
        activation='relu',
        padding='same'
    )(stroke_label_branch)  # (None, 7, 64)

    # Learnable upsampling to reach 180 timesteps
    stroke_label_branch = tf.keras.layers.Conv1DTranspose(
        filters=32,
        kernel_size=3,
        strides=26,  # Adjust stride to fit temporal resolution
        activation='relu',
        padding='same'
    )(stroke_label_branch)  # Approx. (None, 182, 32)

    # Crop or pad to exact shape (None, 180, 32)
    stroke_label_branch = tf.keras.layers.Cropping1D(cropping=(1, 1))(stroke_label_branch)  # (None, 180, 32)

    # Final prediction layer for strokes
    stroke_label_output = tf.keras.layers.Conv1D(
        filters=1,
        kernel_size=1,           # One prediction per timestep
        activation="sigmoid",    # For binary stroke classification
        name="stroke_label_output"
    )(stroke_label_branch)  # (None, 180, 1)
