...
import os
import pickle
from gan_vanilla1 import build_generator, build_discriminator, get_default_generator_parameters, get_default_discriminator_parameters
...
data_parameters = {
    ...
    'win_len': 180,
    'slide_len': 30,
    'window_normalization': 'statistical',
    'label_type': 'majority',
...
# Training loop
def train_gan(generator, discriminator, data_generator, epochs, steps_per_epoch, x_val=None):
    for epoch in range(epochs):
        for step in range(steps_per_epoch):
            ...
            with tf.GradientTape() as tape_gen, tf.GradientTape() as tape_disc:
                ...
                # Discriminator losses with label noise
                real_labels = tf.ones((gan_training_parameters['batch_size'], 1), dtype=tf.float32)
                ...
                # Generator loss with misleading labels
                misleading_labels = tf.ones((gan_training_parameters['batch_size'], 1), dtype=tf.float32)
                g_loss = tf.keras.losses.binary_crossentropy(misleading_labels, fake_preds)
        ...
        if x_val is not None and epoch % 10 == 0:
            ...
            val_fake_data = generator(val_noise, training=False)
